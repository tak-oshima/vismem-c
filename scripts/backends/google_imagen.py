...
class GoogleImagenBackend(ImageGenerator):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if not VERTEX_AI_AVAILABLE:
            raise ImportError("Vertex AI SDK not available. Install: pip install google-cloud-aiplatform")

        credential_path = "google-cloud-key.json"
        if os.path.exists(credential_path):
            os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = credential_path

        project_id = os.getenv("GOOGLE_CLOUD_PROJECT") or os.getenv("GCP_PROJECT_ID") or "your-default-project-id"
    ...
    def simplify_prompt(self, prompt: str) -> str:
        ...
        problematic_words = [
            'real', 'realistic', 'photorealistic', 'hyper-realistic',
            'celebrity', 'person', 'face', 'people', 'human',
            'violence', 'graphic', 'blood', 'gore'
        ]
    ...
    def generate(self, prompt: str, count: int = 1, output_dir: str = 'outputs/images', **kwargs) -> List[Dict[str, Any]]:
        ...
        # Try multiple versions of the prompt with different safety levels
        prompt_variants = [
            (prompt, "block_some"),  # Original prompt, less strict filter
            (self.simplify_prompt(prompt), "block_some"),  # Simplified, less strict
            (self.simplify_prompt(original_prompt), "block_none"),  # Simplified original, no filter
        ]
        ...
        for attempt, (prompt_to_try, safety_level) in enumerate(prompt_variants):
            try:
                logger.info(f"[Attempt {attempt+1}] Generating with prompt: {prompt_to_try[:80]}...")
                logger.info(f"  Safety filter: {safety_level}")
                ...
                if response and hasattr(response, 'images') and response.images and len(response.images) > 0:
                    logger.info(f"✓ SUCCESS on attempt {attempt+1}: Generated {len(response.images)} image(s)")
                    ...
                    logger.warning(f"Attempt {attempt+1} returned empty images: {response}")
                    continue
                ...
                if attempt == len(prompt_variants) - 1:
                    # Last attempt failed
                    logger.error(f"All generation attempts failed for prompt: {original_prompt}")
                    raise
                continue

        logger.error(f"Image generation failed: all attempts returned empty responses")
        return []
